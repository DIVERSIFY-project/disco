...
plot(data$step, data$diversity, 
		ylim=c(0, 1),
		xlim=range(data$step),
		xlab="Time (steps)", 
		ylab="Diversity",
		axes=FALSE, 
		type="n",	
...
i <- 0;
for (run in as.vector(unique(data$controller))) {
    i <- i + 1;
    if (i == 2) {
       i <- i+1	
    }
    lgd <- rbind(lgd, data.frame(pch=i, lty=i, name=run));
    d <- subset(data, controller==run);
    lines(d$step, d$diversity, type="o", lty=i, pch=i);
}
